...
export default function OclcDetailPage() {
  ...
  const displayedFieldRows = useMemo(() => {
    ...
    itemRows.forEach((row, index) => {
      [
        ["Locatie", `[${index}].branchName`, row.location, ENDPOINTS.items],
        ["Uitgave", "imprint", row.publisher, ENDPOINTS.discovery],
        ["Plaats", `[${index}].callNumber`, row.place, ENDPOINTS.items],
        ["Waar te vinden", `[${index}].shelfDescription`, row.whereToFind, ENDPOINTS.items],
        ["Beschikbaarheid", `[${index}].effectiveStatus | [${index}].returnDate`, row.availability, ENDPOINTS.items],
      ].forEach(([label, field, value, endpoint]) => rows.push({
  ...
  }, [author, authorSource, combinedTitle, cover, coverSource, headlineRows, itemRows, practicalRows, recommendationItems, subtitleSource, summary, summarySource, titleSource, topSpecificationRows, volumeNameSource, volumeSource]);
                ...
                <table className="detail-table raw-data-table">
                  ...
                  <tbody>
                    {itemRows.length ? itemRows.map((row) => (
                      <tr key={row.key}>
                        <td><RawValue value={row.location} /></td><td><RawValue value={row.publisher} /></td>
