  ...
  if (Array.isArray(value)) {
    return (
      <ul className="raw-value-list">
        {value.map((entry, index) => <li key={`${rawText(entry)}-${index}`}>{rawText(entry)}</li>)}
      </ul>
    );
  }
  return <span className="raw-value">{hasValue(value) ? rawText(value) : "—"}</span>;
}
    ...
    <section className="specs-list">
      {rows.map((row) => (
        <div className="spec-row" key={`${row.label}-${row.field}`}>
          ...
          <div className="spec-value"><DisplayValue value={row.value} /></div>
        </div>
      ))}
    </section>
  );
            ...
            <div className="card-grid top-cards">
              <section className="info-card">
                ...
                <ul className="raw-specification-values">
                  {topRows.map((row) => (
                    <li key={`${row.label}-${row.field}`}><DisplayValue value={row.value} /></li>
                  ))}
                </ul>
              </section>
                ...
                {view.subjects.length ? (
                  <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
                    {view.subjects.map((subject, index) => (
                      <li key={`${subject}-${index}`} style={{ marginBottom: "10px" }}>{subject}</li>
                    ))}
                  </ul>
          ...
          <section className="table-card displayed-fields-table-card">
            ...
            <div className="table-wrap">
              <table className="detail-table displayed-fields-table">
                <thead><tr><th>Volgorde</th><th>Onderdeel</th><th>Getoonde veldnaam</th><th>OCLC-veldnaam</th><th>Waarde</th><th>Endpoint path</th><th>Opmerking</th></tr></thead>
                <tbody>
                  {usedRows.map((row) => (
                    <tr key={`${row.order}-${row.label}`}>
                      ...
                      <td><code>{row.endpoint}</code></td><td>{row.note}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
