...
const rows = [
  ...
  { field: "Formaat", inputType: "lijst", status: "JA", oclcParameter: "facetFilter", valuePattern: "mediumTypeCode:<code>", source: "mediumtypecode.txt", example: "facetFilter=mediumTypeCode:BOE", note: "Label en code worden uit het lokale WISE-metadatabestand gelezen." },
  { field: "Bibliotheek", inputType: "lijst", status: "JA", oclcParameter: "facetFilter", valuePattern: "branchId:<id>", source: "branch.txt", example: "facetFilter=branchId:1001", note: "De naam is het label; id is de filterwaarde." },
  ...
  { field: "Genre", inputType: "lijst", status: "JA", oclcParameter: "facetFilter", valuePattern: "genreCode:<code>", source: "genrecode.txt", example: "facetFilter=genreCode:DE", note: "Label en code worden uit het lokale WISE-metadatabestand gelezen." },
  { field: "Taal", inputType: "lijst", status: "JA", oclcParameter: "facetFilter", valuePattern: "languageCode:<code>", source: "languagecode.txt", example: "facetFilter=languageCode:GER", note: "Label en code worden uit het lokale WISE-metadatabestand gelezen." },
  { field: "Onderwerp (check)", inputType: "vrij veld", status: "CHECK", oclcParameter: "facetFilter", valuePattern: "subject:<waarde>", source: "invoer", example: "facetFilter=subject:<waarde>", note: "Beschikbaar om de nog te bevestigen waardeconventie te testen." },
  ...
  { field: "Jeugd", inputType: "lijst", status: "JA", oclcParameter: "facetFilter", valuePattern: "targetAudienceCode:<code>", source: "targetaudiencecode.txt", example: "facetFilter=targetAudienceCode:AB", note: "Label en code worden uit het lokale WISE-metadatabestand gelezen." },
...
export function toAdvancedSearchMappingCsv(mappingRows = ADVANCED_SEARCH_MAPPING_ROWS) {
  const columns = [
    ...
    ["source", "Bron keuzelijst"], ["example", "Voorbeeld"], ["note", "Opmerking"],
  ];
